...
whole composition (background, lines, and text). *Alternative considered:* coarser
virtual grain cells scaled to resolution — rejected because at 4K a cell exceeded
a thin contour's width, leaving the lines clean while only the background looked
grainy. *Alternative considered:* `<feTurbulence>` in SVG — rejected: slow at 4K
in resvg, and grain in SVG units stretches with scale.
